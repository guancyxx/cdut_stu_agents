...
# Continue processing tasks that are still waiting in the queue.
...
class JudgeDispatcher(DispatcherBase):
    ...
    def _compute_statistic_info(self, resp_data):
        # Persist the slowest test case resource usage for the submission.
    ...
    def judge(self):
        ...
        if resp["err"]:
            ...
            if not isinstance(test_case_results, list) or not test_case_results:
                ...
                # In ACM mode, the first failing test case decides the final status.
                # In OI mode, partially accepted is allowed when only some cases fail.
        ...
        # The current submission is finished, so try to process queued tasks.
        ...
        with transaction.atomic():
            ...
            if self.contest.rule_type == ContestRuleType.ACM:
                ...
                if problem_id not in contest_problems_status:
                    ...
                    # If the problem is already accepted, skip all counters.
                    return
    ...
    def _update_acm_contest_rank(self, rank):
        ...
        # Reload the problem because its counters may have changed earlier.
        problem = Problem.objects.select_for_update().get(contest_id=self.contest_id, id=self.problem.id)
        # The user has submitted this problem before.
        if info:
            if info["is_ac"]:
                return
        ...
        # The user is submitting this problem for the first time.
        else:
            rank.submission_number += 1
